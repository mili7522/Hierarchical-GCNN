...
#             labels.append(np.floor(float(s[-1]) / 10).astype(int))
    ...
    with open('Data/SA2TTW.csv', 'r') as file:
        for i, line in enumerate(file):
            if i == 0:  # Skip first line (header)
                continue
            s = line[:-1].split(',')
            a = dataset['level_1']['keys'].index(s[0])
            b = dataset['level_1']['keys'].index(s[1])
            dataset['level_1']['adj_mat'][a, b] += float(s[2])/2  # Add symmetrically
            dataset['level_1']['adj_mat'][b, a] += float(s[2])/2
...
dfs = []
dfSaveName = "Results/2018-09-05-JourneyToWorkTest2.csv"
...
expParameters = {"reverseLinkagePosition": "None", "linkagePosition": "Late", "linkageActFun": False, "linkageBatchNorm": True,
                 ...
                 "SA4Graph": False, "auxilaryProjectSA4": False, "SA3Graph": False, "auxilaryProjectSA3": False}
print(exp_number, expParameters)
df = runBatch(expParameters = expParameters, exp_name = "2018-09-05_EXP50_NoneLateGC_JTWAdd")
...
expParameters = {"reverseLinkagePosition": "Early", "linkagePosition": "Late", "linkageActFun": False, "linkageBatchNorm": True,
                 ...
                 "SA4Graph": False, "auxilaryProjectSA4": False, "SA3Graph": False, "auxilaryProjectSA3": False}
print(exp_number, expParameters)
df = runBatch(expParameters = expParameters, exp_name = "2018-09-05_EXP53_EarlyLateGC_JTWAdd")
dfs.append( pd.concat([pd.DataFrame({"ExpNo": [exp_number]*len(df)}), df], axis = 1) )
...
print(exp_number, expParameters)
df = runBatch(expParameters = expParameters, exp_name = "2018-09-05_EXP51_NoneLateGC_SA4SA3_JTWAdd")
dfs.append( pd.concat([pd.DataFrame({"ExpNo": [exp_number]*len(df)}), df], axis = 1) )
...
print(exp_number, expParameters)
df = runBatch(expParameters = expParameters, exp_name = "2018-09-05_EXP52_NoneLateGC_SA3GC_JTWAdd")
dfs.append( pd.concat([pd.DataFrame({"ExpNo": [exp_number]*len(df)}), df], axis = 1) )
...
print(exp_number, expParameters)
df = runBatch(expParameters = expParameters, exp_name = "2018-09-05_EXP54_EarlyLateGC_SA4GCSA3GC_JTWAdd")
dfs.append( pd.concat([pd.DataFrame({"ExpNo": [exp_number]*len(df)}), df], axis = 1) )
...
expParameters = {"reverseLinkagePosition": "Late", "linkagePosition": "Late", "linkageActFun": False, "linkageBatchNorm": True,
                 ...
                 "SA4Graph": False, "auxilaryProjectSA4": False, "SA3Graph": False, "auxilaryProjectSA3": False}
print(exp_number, expParameters)
df = runBatch(expParameters = expParameters, exp_name = "2018-09-05_EXP55_LateLateEmbGC_JTWAdd")
dfs.append( pd.concat([pd.DataFrame({"ExpNo": [exp_number]*len(df)}), df], axis = 1) )
...
expParameters = {"reverseLinkagePosition": "Late", "linkagePosition": "Late", "linkageActFun": False, "linkageBatchNorm": True,
                 ...
                 "SA4Graph": False, "auxilaryProjectSA4": True, "SA3Graph": False, "auxilaryProjectSA3": True}
print(exp_number, expParameters)
df = runBatch(expParameters = expParameters, exp_name = "2018-09-05_EXP56_LateLateEmbGC_SA4SA3_JTWAdd")
dfs.append( pd.concat([pd.DataFrame({"ExpNo": [exp_number]*len(df)}), df], axis = 1) )
...
expParameters = {"reverseLinkagePosition": "Late", "linkagePosition": "Late", "linkageActFun": False, "linkageBatchNorm": True,
                 ...
                 "SA4Graph": True, "auxilaryProjectSA4": True, "SA3Graph": True, "auxilaryProjectSA3": True}
print(exp_number, expParameters)
df = runBatch(expParameters = expParameters, exp_name = "2018-09-05_EXP57_LateLateEmbGC_SA4GCSA3GC_JTWAdd")
dfs.append( pd.concat([pd.DataFrame({"ExpNo": [exp_number]*len(df)}), df], axis = 1) )
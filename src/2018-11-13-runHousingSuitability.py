...
def load_dataset():
    ...
    with open('Data/NewData/2018-11-13-NSW-SA1Input-HousingSuitability-Normalised.csv', 'r') as file:
...
def run(no_folds = 5, supervised = True, i = 0, l = 2, n = 128, expParameters = {}):
    ...
    valid_idx = np.flatnonzero(dataset['level_0']['labels'] >= -9)  # Missing data labelled with -10
    if supervised:
...
dfs = []
dfSaveName = "Results/2018-11-13-HousingSuitability.csv"
...
df = runBatch(expParameters = expParameters, exp_name = "2018-11-13_EXPV2_Housing_NoneLateGC")
...
expParameters = {"reverseLinkagePosition": "Late", "linkagePosition": "Late", "linkageActFun": False, "linkageBatchNorm": True,
                 ...
                 "linkage_adjustment_components": None, "reverse_linkage_adjustment_components": None, "linkage_W_2D": False}
print(exp_number, expParameters)
df = runBatch(expParameters = expParameters, exp_name = "2018-11-13_EXPV5_Housing_LateLateEmbGC")
dfs.append( pd.concat([pd.DataFrame({"ExpNo": [exp_number]*len(df)}), df], axis = 1) )
...
expParameters = {"reverseLinkagePosition": "Early", "linkagePosition": "Late", "linkageActFun": False, "linkageBatchNorm": True,
                 ...
                 "linkage_adjustment_components": None, "reverse_linkage_adjustment_components": None, "linkage_W_2D": False}
print(exp_number, expParameters)
df = runBatch(expParameters = expParameters, exp_name = "2018-11-13_EXPV3_Housing_EarlyLateGC")
dfs.append( pd.concat([pd.DataFrame({"ExpNo": [exp_number]*len(df)}), df], axis = 1) )
...
expParameters = {"reverseLinkagePosition": "Late", "linkagePosition": "Late", "linkageActFun": False, "linkageBatchNorm": True,
                 ...
                 "linkage_adjustment_components": None, "reverse_linkage_adjustment_components": None, "linkage_W_2D": False}
print(exp_number, expParameters)
df = runBatch(expParameters = expParameters, exp_name = "2018-11-13_EXPV4_Housing_LateLateGC")
dfs.append( pd.concat([pd.DataFrame({"ExpNo": [exp_number]*len(df)}), df], axis = 1) )
...
expParameters = {"reverseLinkagePosition": "None", "linkagePosition": "None", "linkageActFun": False, "linkageBatchNorm": True,
                 "linkageNeurons": None, "auxilaryEmbedding1": False, "auxilaryEmbedding2": False, "auxilaryGraph": None,
                 "linkage_adjustment_components": None, "reverse_linkage_adjustment_components": None, "linkage_W_2D": False}
print(exp_number, expParameters)
df = runBatch(expParameters = expParameters, exp_name = "2018-11-13_EXPV0_Housing_None")
dfs.append( pd.concat([pd.DataFrame({"ExpNo": [exp_number]*len(df)}), df], axis = 1) )
...
expParameters = {"reverseLinkagePosition": "None", "linkagePosition": "Late", "linkageActFun": False, "linkageBatchNorm": True,
                 ...
                 "linkage_adjustment_components": None, "reverse_linkage_adjustment_components": None, "linkage_W_2D": False}
print(exp_number, expParameters)
df = runBatch(expParameters = expParameters, exp_name = "2018-11-13_EXPV1_Housing_NoneLate")
dfs.append( pd.concat([pd.DataFrame({"ExpNo": [exp_number]*len(df)}), df], axis = 1) )

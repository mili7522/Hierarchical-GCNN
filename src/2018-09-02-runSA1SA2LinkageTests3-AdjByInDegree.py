...
#             labels.append(np.floor(float(s[-1]) / 10).astype(int))
    ...
    adj_mat_in_degree = np.sum(adj_mat_SA2, axis = -1)
    for i, n in enumerate(adj_mat_in_degree):  # Broadcasting this column array seem to give memory problems
        if n == 0:
            continue
        adj_mat_SA2[i] = adj_mat_SA2[i] / n
    ...
    adj_mat_in_degree = np.sum(adj_mat_SA1SA2, axis = -1)
    for i, n in enumerate(adj_mat_in_degree):  # Broadcasting this column array seem to give memory problems
        if n == 0:
            continue
        adj_mat_SA1SA2[i] = adj_mat_SA1SA2[i] / n
    
    adj_mat_in_degree = np.sum(adj_mat_SA2SA1, axis = -1)
    for i, n in enumerate(adj_mat_in_degree):  # Broadcasting this column array seem to give memory problems
        if n == 0:
            continue
        adj_mat_SA2SA1[i] = adj_mat_SA2SA1[i] / n
...
### Combine and output
df = pd.concat(dfs)
df.to_csv("2018-09-02-TestResults3.csv")
